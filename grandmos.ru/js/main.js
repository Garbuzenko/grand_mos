...
function popupWindowBottomFunc(content,width,height,zIndex) {
    ...
    var cl = $('#'+popupWindow).clone();
    cl.addClass('popupWindowsBottom');
    cl.find('#'+popupWindow+'SubDiv').css({'height':height}).html(content);
    cl.find('.jsClosePopupWindow').removeClass('popupCloseDiv').addClass('popupCloseDivBottom');
...
$('body').on('keyup', '.jsSelectList', function() {
    ...
    var value = $('#' + obj + '_value').val();
       ...
       $.post("/", {search: search, value: value, action: "ajax", field: obj, module: mod, component: com},
        	function (html) {
     	      if (html!='') {
...
$('body').on('click', '.jsChangeFontSize', function() {
        ...
        if (st == 'big') {
            ...
            $("body").css({"zoom":"100%"}); 
            $('#jsHeaderLogo').removeClass('hidden');
            $('#jsCabinet').css({"width":"50px","height":"50px","font-size":"21px","line-height":"50px"}); 
            $('#jsFontSizeBig').removeClass('active');
            t.addClass('active'); 
        }
        
        Cookies.set('fontSize', st);
...
$('body').on('click', '.send_form', function() {
        ...
        if (arr['is_ajax'] != 1) {
                ...
                if (arr['preloader'] != false) {
                    $('#'+arr['preloader']).removeClass('hidden');
                }
                ...
                $.ajax({
                       ...
                       if (isJSON(html) == true) {
                        ...
                        if (arr['ajaxLoadAnimation'] != false) {
                            $('#'+arr['ajaxLoadAnimation']).html(html).css("display", "none").fadeIn(400);
                            html = 'ok';
                        }
                        
                        if (arr['ajaxLoadNotEmpty'] != false) {
                           if (html != '') {
                              $('#'+arr['ajaxLoadNotEmpty']).html(html);
                           }
                        }
