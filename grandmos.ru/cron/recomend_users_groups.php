...
//Обновить интересы
// INSERT INTO interests(user_id, online, field_name, field_value, count_lessons) SELECT user_id, online_id,'level1', level1, sum(COUNT)/3 FROM attend_stat GROUP by user_id, online_id, 'level1', level1;
// INSERT INTO interests(user_id, online, field_name, field_value, count_lessons) SELECT user_id, online_id,'level2', level2, sum(COUNT)/2 FROM attend_stat GROUP by user_id, online_id, 'level2', level2;
...
if ($users != false) {
    foreach($users as $user) {
            ...
            $add = db_query("INSERT INTO interests_friend(user_id,online, field_name, field_value, count_lessons)  
                             SELECT f.user_id, online, field_name, field_value, ROUND(SUM(f.count_lessons + i.count_lessons)/COUNT(f.friend_user_id)) AS count_lessons FROM friends as f lEFT JOIN interests as i ON i.user_id = f.friend_user_id 
                             WHERE f.user_id = ".$user['user_id']."
            GROUP BY field_name, field_value DESC","i");

            //Подставить максимальный радиус, в который сейчас ходит + 1 километр и Добавим 30% к дистанции
            $radius = 2000 + $user['distance']*1.3; 
            ...
            AND groups.test = 1 
            GROUP BY group_id";
            // ORDER BY user_interests DESC, ((age_gender + 0.1) / (distance + 500)) * online_offline  DESC ";
            $ins = db_query($sql,"i");  
            echo $sql;
            echo '<br>';
...
//Посчитаем итоговую рекомендацию для офлайн
            $sql = "UPDATE recomend_users_groups as r SET recommend = 
                                  ( age_gender * " . $k_const['k_age_gender'] . " 
                                     ...
                                     )*online_ofline/100/distance*(1+distance_bonus*".$k_const['k_distance'].")
